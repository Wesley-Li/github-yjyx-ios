...
-(IBAction)registBtn:(id)sender
{
    if (textField1.text.length == 0||textField2.text.length == 0||textField3.text.length == 0||textField4.text.length == 0) {
        ...
        [self.view makeToastActivity:SHOW_CENTER];
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:codeTextField.text,@"code",textField1.text,@"name",textField2.text,@"password",textField3.text,@"phone",textField4.text,@"relation",childrenCid,@"cid",@"",@"sessionid",@"1",@"ostype",((AppDelegate *)SYS_DELEGATE).deviceToken,@"devicetoken",[[UIDevice currentDevice] model],@"description",nil];
