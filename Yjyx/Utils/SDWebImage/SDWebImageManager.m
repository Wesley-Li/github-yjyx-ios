...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    {
        if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)]) {
            [delegate webImageManager:self didFinishWithImage:image forURL:url];
        }
//        objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:), self, image, url);
    ...
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
    {
        ...
        if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)]) {
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            {
                if (delegate && [delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:)]) {
...
//                objc_msgSend(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:), self, image, downloader.url);
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:)])
            {
                ...
                if (delegate && [delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:)]) {
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                {
                    if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:)]) {
                        [delegate webImageManager:self didFinishWithImage:image forURL:downloader.url];
                    }
...
//                    objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:userInfo:), self, image, downloader.url, userInfo);
            ...
            else
            {
                ...
                {
                    if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:)]) {
                        [delegate webImageManager:self didFailWithError:nil forURL:downloader.url];
                    }
//                    objc_msgSend(delegate, @selector(webImageManager:didFailWithError:forURL:), self, nil, downloader.url);
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)])
                {
                    ...
                    if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFailWithError:forURL:userInfo:)]) {
                        [delegate webImageManager:self didFailWithError:nil forURL:downloader.url userInfo:userInfo];
                    }
