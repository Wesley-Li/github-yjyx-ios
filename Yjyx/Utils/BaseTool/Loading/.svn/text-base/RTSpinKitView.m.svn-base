...
-(instancetype)initWithStyle:(RTSpinKitViewStyle)style {
    return [self initWithStyle:style color:[UIColor grayColor] on:nil center:(CGPoint){0,0}];
}
...
-(instancetype)initWithStyle:(RTSpinKitViewStyle)style color:(UIColor*)color on:(UIView*)parent center:(CGPoint)point {
    ...
    if (self) {
        ...
        if (style == RTSpinKitViewStylePlane) {
            ...
            CGFloat barWidth = CGRectGetWidth(self.bounds) / 30;
    
            for (NSInteger i=0; i < 30; i+=1) {
...
-(CGSize)sizeThatFits:(CGSize)size {
    if (_style == RTSpinKitViewStyleWave) {
        return CGSizeMake(150, 10);
    }
    if (_style == RTSpinKitViewStylePlane) {
